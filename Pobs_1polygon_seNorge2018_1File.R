...
nc <- nc_open(filename = "./seNorge2018_RR_1957_2017.nc", verbose = F, write = FALSE)
nc$var$projection_utm
...
nc$var$rr
ts=3
Prec_ts = ncvar_get(nc, varid= varname, start = c(1,1,ts), count=c(-1,-1, 1), verbose=FALSE,
                    signedbyte=TRUE, collapse_degen=TRUE)
tic()
avg_Prec_idx <- round(mean(Prec_ts[indexlist[[1]]]), digits = 2)
toc()
...
df_Prec <- cbind(timeseries, df_Prec)
colnames(df_Prec)[1] <- "SUBID"

#check nc file name
nc$filename
filename =  paste0("POBS_",str_remove(nc$filename,"./"), "_Gaula_247.txt")
write.table(df_Prec, file = filename, quote = F, row.names = F, sep = "\t")
file.show(filename)
